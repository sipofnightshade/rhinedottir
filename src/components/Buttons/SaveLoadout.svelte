<script lang="ts">
  ...
  let btnState: 'disabled' | 'warning' | 'success' = 'success';
  let title: string;
  ...
  >
    ...
    >
      <path
        ...
        class:fill-emerald-500={btnState === 'success'}
        class:fill-yellow-500={btnState === 'warning'}
      />
    </svg>
  </div>
  <span
    ...
    class:text-emerald-500={btnState === 'success'}
    class:text-yellow-500={btnState === 'warning'}>Save</span
  >
</button>
<!-- @component
- Open Modal here and put the save Logic inside of it.
- The save logic is just calling the loadouts.addLoadout method
 -->
<ShortModal bind:dialog modalTitle="Save Character Loadout">
  <label for="input">
    ...
    <input
      ...
      bind:value={title}
      class="h-8 w-full rounded-sm bg-slate-600 text-slate-200"
    />
  </label>
  <button
    on:click={saveLoadout}
    class="mt-2 ml-auto rounded-full bg-emerald-600 py-2 px-4">Save</button
  >
</ShortModal>
